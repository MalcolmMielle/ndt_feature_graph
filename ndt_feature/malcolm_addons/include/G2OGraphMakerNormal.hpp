#ifndef NDTFEATURE_G2OGRAPHMAKERNORMAL_27072016
#define NDTFEATURE_G2OGRAPHMAKERNORMAL_27072016
...
namespace ndt_feature {
	...
	public:
		// :( I hate pointers
		G2OGraphMarkerNormal(const g2o::SE2& sensoffset, 
...
// 				g2o::SE2 se2(_landmark_positions[i](0), _landmark_positions[i](1), _landmark_positions[i](2));
				Eigen::Vector3d vec_tmp = _landmark_positions[i].toVector();
				Eigen::Vector2d vec;
			...
			for (size_t i = 0; i < _observation_real_landmarks.size(); ++i) {
				...
				std::cout << "LAndmark s: " << std::get<1>(_observation_real_landmarks[i]) << " " << std::get<2>(_observation_real_landmarks[i]) << std::endl;
...
// 				g2o::SE2 se2(_prior_landmark_positions[i](0), _prior_landmark_positions[i](1), _prior_landmark_positions[i](2));
				Eigen::Vector3d vec_tmp = _prior_landmark_positions[i].toVector();
				Eigen::Vector2d vec;
				vec << vec_tmp(0), vec_tmp(1);
				landmark->setEstimate(vec);
				landmark->setEstimate(vec);
			...
			for (size_t i = 0; i < _edges_prior.size(); ++i) {
				...
				Eigen::Vector3d meas = std::get<0>(_edges_prior[i]).toVector();
				Eigen::Vector2d meas2d;
				meas2d << meas(0), meas(1);
				landmarkObservation->setMeasurement(meas2d);
